...
int main(int argc, char * argv[]) {
    ...
    std::vector<double> ps {0.52778};
    std::vector<int> runs {500};

    for (int i=0; i<ps.size(); i++) {
        // TreePlusP ensemble2(number_nodes, sc, trc, rng, ps[i]);
        ...
        std::set<Alg> s {Alg::Optimal, Alg::Optimal2};//, Alg::Schranke4b, Alg::GreedyApprox};
        // std::string ordner_name = "TimeTest_" + std::to_string(number_nodes) + "_nodes_" + std::to_string(number_scenarios) + "_scenarios";
        std::string ordner_name = "OptimumVSOptimum" + std::to_string(number_nodes) + "_nodes_" + std::to_string(number_scenarios) + "_scenarios";
